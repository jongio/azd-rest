 ...
 */
function parseHelpOutput(helpText: string): { description: string; usage: string; flags: Flag[] } {
  ...
  // Parse Usage: line — handles both "Usage: cmd [flags]" and multiline format:
  //   Usage:
  //     cmd [flags]
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith('Usage:')) {
      const sameLine = lines[i].replace('Usage:', '').trim();
      if (sameLine) {
        usage = sameLine;
  ...
  for (const line of lines) {
    ...
    if (inFlags && line.trim()) {
      ...
      // or with type:         "  -d, --data string    Request body (JSON string)"
      // Cobra format: optional short, long name, optional type, description
      const match = line.match(/^\s+(-\w)?,?\s*(--[\w-]+)\s+(.+)$/);
      if (match) {
        let desc = match[3].trim();
        // Strip leading type indicator from description (e.g., "string   Description" -> "Description")
        desc = desc.replace(/^(?:string(?:Array)?|int|duration|float\d*)\s{2,}/, '');
        // Skip the --help flag (noise on individual command pages)
        if (match[2] === '--help') continue;
        flags.push({
          short: match[1] || '',
          flag: match[2],
          description: desc
        ...
        if (longMatch) {
          let desc = longMatch[2].trim();
          desc = desc.replace(/^(?:string(?:Array)?|int|duration|float\d*)\s{2,}/, '');
          if (longMatch[1] === '--help') continue;
          flags.push({
            short: '',
            flag: longMatch[1],
            description: desc
  ...
  return { description, usage, flags };
}
        ...
        if (match && !['completion', 'help'].includes(match[1])) {
          ...
          try {
            ...
            commands.push({
              name: cmdName,
              description: cmdDesc,
              usage: parsed.usage || `azd rest ${cmdName} [flags]`,
              flags: parsed.flags
            });
          } catch {
            commands.push({
...
/** Escape HTML entities and curly braces for Astro/JSX template output */
...
${commandCards}
  ...
  --scope https://myservice.com/.default\`} language="bash" />
      ...
      <section class="section">
        ...
        <p>
          For non-Azure endpoints, use <code>--scope</code> to provide the scope manually,
          or <code>--no-auth</code> to skip authentication entirely.
        </p>
      </section>
    </div>
  </div>
