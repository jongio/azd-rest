...
- ✅ No URL validation beyond basic parsing
- ✅ Can access internal network endpoints
- ✅ Can access cloud metadata endpoints (AWS, Azure IMDS)
- ✅ Can bypass firewalls if running on internal network
...
**Why This Works**:
- ✅ Metadata services accessible from within cloud instances
- ✅ Often contain sensitive credentials and tokens
- ✅ No authentication required from localhost
- ✅ User may not realize they're accessing metadata
...
**Why This Works**:
- ✅ No rate limiting in azd-rest
- ✅ User can make unlimited requests
- ✅ Could exhaust Azure API quotas
- ✅ Could consume local network/CPU resources

**Mitigation**: Operating system and network limits apply
...
- ✅ Large responses loaded into memory
- ✅ Could cause OOM (Out of Memory) errors

**Mitigation**: HTTP client has default limits, OS memory limits apply
...
|--------------|------------|--------|------------|------------|
| SSRF - Internal Network | **HIGH** | **CRITICAL** | 🔴 CRITICAL | ❌ No (by design) |
| SSRF - Cloud Metadata | **HIGH** | **CRITICAL** | 🔴 CRITICAL | ❌ No (by design) |
| SSRF - Internal Services | MEDIUM | HIGH | 🟡 HIGH | ❌ No (by design) |
...
| Header Injection | LOW | MEDIUM | 🟢 LOW | ⚠️ Partial |
...
❌ **Design Security**: Vulnerable to SSRF
- **Users explicitly provide URLs (by design)**
- **SSRF is an inherent risk of making HTTP requests**
- **No URL validation beyond basic parsing**
- **Internal network and metadata endpoints accessible**
...
1. **CRITICAL**: Add SSRF warnings to README and documentation
2. **HIGH**: Implement optional URL validation (block internal IPs by default)
3. **MEDIUM**: Add audit logging
4. **LOW**: Consider rate limiting for future versions
