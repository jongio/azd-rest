...
| `--count` | | bool | false | Print the number of records in a JSON response and nothing else. Runs after `--query`; cannot be combined with `--template` or `--no-body`. |
| `--template` | | string | "" | Render the JSON response through a Go `text/template`, or use `@file` to load one. Runs after `--query`; cannot be combined with `--count` or `--no-body`. |
...
| `--no-body` | | bool | false | Discard the response body after the request while keeping status, header, and write-out metadata. Cannot be combined with `--template` or `--count`. |
